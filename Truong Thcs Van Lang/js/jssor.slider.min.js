(function(g, e, b, j, c, i, k) {
    ...
    var a = g.$Jssor$ = new function() {
        ...
        function Ib() {
            if (!I) {
                ...
                var a;
                if (y.pointerEnabled || (a = y.msPointerEnabled))
                    I.$TouchActionAttr = a ? "msTouchAction" : "touchAction"
            }
            return I
        }
        function t(i) {
        ...
        f.$DeselectElements = function() {
            if (e.selection)
                ...
                g.getSelection && g.getSelection().removeAllRanges()
        }
        ;
        ...
        f.$Cast = function(h, i, w, n, y, z, o) {
            ...
            if (c.$Clip && a.$CanClearClip() && !c.$Clip.$Top && !c.$Clip.$Left && c.$Clip.$Right == o.$OriginalWidth && c.$Clip.$Bottom == o.$OriginalHeight)
                c.$Clip = j;
            return c
        }
    }
    ...
    var h = g.$JssorSlider$ = function(q, fc) {
                    ...
                    if (j == "caption") {
                        ...
                        if (h == "A") {
                            if (a.$AttributeEx(f, "u") == "image")
                                n = a.$FindChildByTag(f, "IMG");
                            else
                                n = a.$FindChild(f, "image", c);
                            if (n) {
                                w = f;
                        ...
                        } else if (h == "IMG" && a.$AttributeEx(f, "u") == "image")
                            n = f;
                        if (n) {
                            n.border = 0;
                            a.$SetStyles(n, V)
                ...
                if (!B && !O && !r && s == f) {
                    ...
                    if (a != d)
                        if (a == e)
                            ...
                            i = b.$GetPlayToPosition();
                    o.$TriggerEvent(p, f, a, m, j, e, d);
                    var l = P && (!E || F);
                    if (a == d)
                        (e != d && !(E & 12) || l) && n.$GoForNextSlide();
                    else
                        (l || a != e) && b.$PlayToPosition(i, w)
                }
            }
            ;
        ...
        if (f.$Idle != k)
            f.$AutoPlayInterval = f.$Idle;
        if (f.$DisplayPieces != k)
            f.$Cols = f.$DisplayPieces;
        if (f.$ParkingPosition != k)
            f.$Align = f.$ParkingPosition;
        ...
        f.$ArrowKeyNavigation && a.$AddEvent(e, "keydown", function(a) {
            if (a.keyCode == 37)
                lb(-f.$ArrowKeyNavigation);
            else
                a.keyCode == 39 && lb(f.$ArrowKeyNavigation)
        });
        var kb = f.$StartIndex;
        if (!(D & 1))
            kb = b.max(0, b.min(kb, r - u));
    ...
    g.$JssorThumbnailNavigator$ = function(g, C) {
        ...
        if (d.$DisplayPieces != k)
            d.$Cols = d.$DisplayPieces;
        if (d.$Rows != k)
            d.$Lanes = d.$Rows;
        A = a.$CssWidth(g);
        y = a.$CssHeight(g);
